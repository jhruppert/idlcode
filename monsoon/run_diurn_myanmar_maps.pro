; 
; Diurnal composite maps of Myanmar WRF output.

; James Ruppert
; 8/14/20
; 
pro run_diurn_myanmar_maps
...
nday_avg=5 ; will composite over last 'nday_avg' days
...
;      v=reform(read_nc_var(file,'',varid='0',count=count,offset=offset))
    ...
    count=[dims.nx,dims.ny,1,i_nt] & offset=[0,0,0,0] ; x,y,z,t
    var=reform(read_nc_var(file,iv_str,count=count,offset=offset)) ; if rainrate, mm/d, else mm
...
vardc=fltarr(dims.nx,dims.ny,npd)
udc=vardc
vdc=vardc
...
for it=0,npd-1 do begin
  ...
  vardc[*,*,it]=mean(var[*,*,t_ind],dimension=3,/double,/nan)
