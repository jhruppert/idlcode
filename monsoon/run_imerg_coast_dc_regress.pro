; 
; Regression analysis of diurnally varying rainfall (Myanmar/Meiyu IMERG data) and ERA5
; using area-averaged rainfall time series calculated in run_imerg_coast_index.pro
;
; James Ruppert
; 12/07/20
; 
pro run_imerg_coast_dc_regress
...
;----DATA DIRECTORIES--------------------
  ...
  im_fil_dm=maindir+'imerg/data/jjas_2013-2017_daymean_3B-HHR.MS.MRG.3IMERG.V06B.nc4'
...
;----READ IMERG RAINFALL--------------------
  ...
  rain = read_imerg_dc_jjas(maindir, rain_dc=rain_dc, lon=lonim, lat=latim) ; [x,y,hr,day] returns mm/day
  nx=n_elements(lonim)
  ny=n_elements(latim)
...
if var_plot eq 'rain' then begin
  ...
  setmax=30 & setmin='0.'
  cbform='(i2)'
  var_mn=mean(mean(rain,dimension=3,/nan,/double),dimension=3,/nan,/double)
  lon=lonim
  lat=latim
  ...
  figname=figdir+'imerg_mean3_'+var_plot+'_'+dat_str
...
;----RAINFALL INDICES--------------------
  ...
  irain_smd -= mean(irain_smd,/nan,/double)
  irain_cdiff -= mean(irain_cdiff,/nan,/double)
  ...
  std_smd=sqrt( total(irain_smd^2,/nan,/double)/(nd-1) )
  std_cdiff=sqrt( total(irain_cdiff^2,/nan,/double)/(nd-1) )
...
tic
  calc_dc_regress, rain, $
    irain_all, irain_coast, irain_offshore, irain_onshore, irain_smd, irain_cdiff, $ ; input indices
