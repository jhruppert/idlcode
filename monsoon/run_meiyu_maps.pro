...
;LEVEL SELECTION
  psel=700 ;700 is the winner of 600, 700, 800, 850, 925
...
;    ndavg=5
;    var=reform(var[*,*,i_nt-2] - var[*,*,i_nt-2-ndavg*npd])
...
;    varm=total(var[*,*,t_ind],3,/double)
    ...
    ;AVERAGE AVOR OVER LAST n DAYS OF TIME SERIES
;    ndavg=2
;    t_ind=indgen(ndavg*npd)+i_nt-1-ndavg*npd
;    var=mean(temporary(var[*,*,t_ind]),dimension=3,/double,/nan)
  endif else begin
...
;----CREATE PLOTS--------------------
  ...
  meiyu_figspecs, var_str, figspecs, setmin=setmin, setmax=setmax, set_cint=5
...
;  figspecs.title+=' ('+strupcase(dirs.cases[ic]);+')'
  ...
  ;LOOP THROUGH TIMES
  iskip=6;1;6;24;12 ; every x hrs
  for it=0,i_nt-1,iskip do begin
;  for it=80,80 do begin
...
;    t_stamp=string(hh,format='(i2.2)')+' L'
    ...
    wrf_meiyu_map_plot, dirs, ivar, dims.lon, dims.lat, figspecs, wind=wind;, cvar=cvar
