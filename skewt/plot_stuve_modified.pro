...
;-
PRO plot_stuve_modified,w_int=w_int,plot_thick=plot_thick,pres=pres,Temp=Temp,td=td,col_temp=col_temp,$
  col_td=col_td,wspeed=wspeed,wangle=wangle,title=title,$
...
Pstartlab1=1000     & Pendlab1=200. & Psteplab1=-50.
Pstartlab2=Pendlab1 & Pendlab2=Plo  & Psteplab2=-25.
;endif
...
Tstart=Tlo       &  Tend=Thi          &  Tstep=20.
Tstartlab=Tlo    &  Tendlab=Thi       &  Tsteplab=10.;Tstep
Pstart=Phi       &  Pend=Plo          &  Pstep=-50.
thetastart=190.  &  thetaend=450.     &  thetastep=20.
thetaestart=190. &  thetaeend=400.    &  thetaestep=20.

;  Set up plot area
;IF windok THEN xhi=0.89 ELSE 
xhi=0.95
xlo=0.08  &  ylo=0.07 &  yhi=0.95
...
for T = Tstart, Tend, Tstep do begin
    plots, [T,T], [Pylo,Pyhi],  thick=plot_thick
endfor
Pylabel = Pyhi + (Pyhi-Pylo)*0.025
for T = Tstartlab, Tendlab, Tsteplab do begin
    xyouts, T,Pylabel+0.06, alignment=0.5, orient=0, /noclip, $
    charsize=1.5*labsize, charthick=plot_thick, string(T,format='(I3)')
endfor

;  Make the pressure lines and label them
for P = Pstartlab1, Pendlab1, Psteplab1 do begin
    Py=P^0.286
    plots, [Tlo,Thi], [Py,Py], thick=plot_thick
endfor
for P = Pstartlab2, Pendlab2, Psteplab2 do begin
    Py=P^0.286
    plots, [Tlo,Thi], [Py,Py], thick=plot_thick
endfor
for P = Pstartlab1, Pendlab1, Psteplab1 do begin
    ...
    charsize=1.5*labsize, charthick=plot_thick, string(P,format='(I4)')
endfor
for P = Pstartlab2, Pendlab2, Psteplab2 do begin
    Py = P^0.286 + (Pyhi-Pylo)*0.005
    xyouts, Tlo, Py, alignment=1.1, orient=0, /noclip,  $
...
for theta = thetastart, thetaend, thetastep do begin
    ...
    Py = Parr^0.286
    plots, Tarr, Py, noclip=0, linestyle=0, thick=2*plot_thick, color=200
endfor
...
for theta = thetaestart, thetaeend, thetaestep do begin
   ...
   plots, Tarr, Pyarr, noclip=0, linestyle=0, thick=2*plot_thick, color=90
endfor

;plots, [0.25,0.30], [0.050,0.050], /normal, linestyle=0
;xyouts, 0.31, 0.045, alignment=0.0,  '!5Dry Lapse Rate', $
;  charsize=1.2*legendsize,charthick=1.5, /normal
;plots, [0.55,0.60], [0.050,0.050], /normal, linestyle=2
;xyouts, 0.61, 0.045, alignment=0.0,  '!5Moist Lapse Rate', $
;  charsize=1.2*legendsize,charthick=1.5, /normal
...
IF nlev GT 0 THEN BEGIN
  ...
  IF n_elements(temp) EQ nlev THEN BEGIN
     ...
     IF n_elements(col_temp) NE 0 THEN $
        plots, temp, Pyarr, noclip=0, linestyle=linestyle, thick=6.,color=col_temp $
  ...
  IF n_elements(td) EQ nlev THEN BEGIN
     ...
     IF n_elements(col_td) NE 0 THEN $
        plots, td, Pyarr, noclip=0, linestyle=linestyle, thick=6.,color=col_td $
...
IF windok THEN BEGIN
   ...
   FOR i=0,n_elements(index)-1,w_int DO wind_barb,wspeed[index[i]],wangle[index[i]],$
     p[0,index[i]],p[1,index[i]],size=1.5,/normal,/nocircle
ENDIF
