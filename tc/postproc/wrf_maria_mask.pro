...
; 4/16/19
; 
function wrf_maria_mask, invar, time, hurdat, dims
...
;print,n_elements(where(finite(outvar[*,*,t_prior])))
  ...
  if ntprior gt 0 then begin
    ...
    ynan=where((dims.lat ge 14) or (dims.lat le 6),nnan,complement=ngood)
    if ndim eq 3 then outvar[*,ynan,t_prior]=!values.f_nan
    if ndim eq 4 then outvar[*,ynan,*,t_prior]=!values.f_nan
    xnan=where(dims.lon le -52,nnan,complement=ngood)
...
;OTHER SIMPLE FILTERS
  ...
  it_xnan=where(time gt julday(9,14,2017,12+10,0,0),count)
  ;indgen(ntvar-10)+10
  if count gt 0 then begin
    xnan=where(dims.lon gt -36,nnan,complement=ngood)
