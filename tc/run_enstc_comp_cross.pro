; 
; BINNED CROSS SECTIONS FOR ENSEMBLE TC RUNS
...
icont=3 ; 0=stream, 1=cloud, 2=LH, 3=AVOR, 4=W
...
;EXCLUDE EDGES
  ...
  ix_skip = indgen(nxskip)+ixskip
  iy_skip = indgen(nyskip)+ixskip
  ;LOWER CORNER
...
;  file = file_name1+'memb_'+string(imem,format='(i2.2)')+'/'+case_str+'/'+file_name2
  ...
  ;TIME SUBSET
    it_read = where((dims.time ge time_ctl[0]) and (dims.time le max(time_ctl)),nttest)
    if nttest eq 0 then continue

  ;count=[dims.nx,dims.ny,1,nt_test] & offset=[0,0,0,it0] ; x,y,z,t
  count=[nxskip,nyskip,1,nttest] & offset=[ix_skip[0],iy_skip[0],0,it_read[0]] ; x,y,z,t
...
;    endif
  ...
  ;OLR
    olrstr='LWUPT'
    iv=where(vars.vars eq olrstr)
    file=dirs.files_post[ic,iv]
    olr=reform(read_nc_var(file,olrstr,count=count,offset=offset))
...
;    olrc=reform(read_nc_var(file,'OLRC',count=count,offset=offset))
  ...
  ;MAIN VARS
    ...
    if var1_str eq 'RH' then begin
      ...
      count=[nxskip,nyskip,dims.np,nttest] & offset=[ix_skip[0],iy_skip[0],0,it_read[0]] ; x,y,z,t
