; 
; Plot time series from azimuthally averaged TC output (generated by run_calc_azim.pro).
;
; FROM COMP_CROSS, NOT UPDATED HERE: Mean water vapor profile used for experiment 'FIXQV' generated here (average over hr_sel=[0,24*2]).
;
; James Ruppert
; 4/6/19
; 
pro run_tc_azim_tser
...
;  hr_plot=[61,84]
  hr_plot=[24,120]
  hr_plot=[0,144]
  if tcname eq 'haiyan' then hr_plot=[24,144]
  hr_tag_full=string(hr_plot[0],format='(i3.3)')+'-'+string(hr_plot[1],format='(i3.3)')+'hr'
...
;AZIM FILES
  ...
  if strmatch(subdir,'*redux*') then hr_sel=[0,144]
...
var1_tser=fltarr(dirs.nc,nt_sav)
